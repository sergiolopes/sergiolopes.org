...
The old `palestras.html.md` page has 102 first-level event records from 2005–2019 in `src/data/talks-history.json`, including 46 entries that had no link in the source. The six nested video/interview bullets are attached to their parent events, so the record retains 70 source links in total. `src/data/talks-enrichments.json` adds the public InfoQ recording found for the 2019 QConSP entry without duplicating that event. Eight later public records and two personal records are kept in `src/data/talks-new.json`; possible TDC homonyms remain intentionally held for confirmation. The `/palestras/ano/YYYY/` indexes and the main page present both sets together in one chronology grouped by year.
...
- A link audit (`npm run check:links`) covers all 117 source post files, 42 document files, the 102-record talk baseline, generated HTML/CSS references, and the recovered talk materials. When `../blog-source` is present it compares talk source lines, year counts, no-link counts, and every source link; from a fresh clone it still checks the committed 102-record baseline. External URLs are intentionally excluded from the local path check.
